...
RSpec.describe 'get /forecast' do
  ...
  it 'returns json data to plan for munchies', :vcr do 
    ...
    expect(munchie_response[:data][:attributes][:travel_time]).to eq("01:45:28")
    expect(munchie_response[:data][:attributes][:forecast]).to be_a Hash
    expect(munchie_response[:data][:attributes][:forecast][:summary]).to eq("few clouds")
    expect(munchie_response[:data][:attributes][:forecast][:temperature]).to eq("45.32")
